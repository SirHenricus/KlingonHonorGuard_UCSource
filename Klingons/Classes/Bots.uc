...
var float WeaponDamageScale;	// Quad Damage

var	BOOL	bBumpTimer;	// Used in roaming
var	BOOL	bRoamTimer;
...
function eAttitude AttitudeTo(Pawn Other)
{
	if (Other.bIsPlayer)
	{
		if ( (Level.Game.IsA('TeamGame') && (Team == Other.Team)) || (bIsWingman))
...
simulated function PlayFootStep()
{
	...
	{
//xxx		PlaySound(sound 'LSplash', SLOT_Interact, 1, false, 1500.0, 1.0);
		return;
	}
...
ignores EnemyNotVisible;
	...
	function Timer()
	{
		...
		foreach VisibleActors (class 'Pawn', A, 1000) {
			if (A!=self && A!=owner && A.bIsPlayer==FALSE) {
				Enemy = A;
				//Log("Enemy - " $Enemy);
			}
		}
