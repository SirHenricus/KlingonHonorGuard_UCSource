...
//	aimerror = aimerror * (2.4 - 0.5 * (skill + FRand()));	
	aimerror = aimerror * (5.4 - 0.5 * ((skill*3) + FRand()));	
...
function bool SetEnemy( Pawn NewEnemy )
{
	...
	if (NewEnemy.bIsPlayer==TRUE && (NewEnemy.bHidden || NewEnemy.Style==STY_Translucent)) {
		//Log ("Player is hidden");
		return FALSE;
	}
...
ignores SeePlayer, HearNoise, Bump, HitWall;
	...
	function ChooseAttackMode()
	{
		...
		//log("Attitude To Enemy - " $AttitudeToEnemy);
	...
	if ( bHasRangedAttack && AlarmNode(OrderObject).bAttackWhilePaused )
	{
		...
		if ( AlarmNode(OrderObject).AlarmAnim != '')
		{
			...
			if ( AlarmNode(OrderObject).AlarmSound != None)
				PlaySound( AlarmNode(OrderObject).AlarmSound);
