...
// DeathMatchGame.
//=============================================================================
class DeathMatchGame expands KlingonGameInfo
	localized
	config;
...
function bool RestartPlayer( pawn aPlayer )	
{
	...
	if( bRestartLevel && Level.NetMode!=NM_DedicatedServer && Level.NetMode!=NM_ListenServer && !bMultiPlayerBots )
		return true;
...
function PostBeginPlay()
{
	...
	Log(Self$".PostBeginPlay() bChangeLevels="$bChangeLevels);
	
	BotConfig = spawn(class'KlingonBotInfo');
	RemainingTime = 60 * TimeLimit;
/*
	KlingonHUD(PlayerPawn(Owner).myHUD).SetTimeLimit(RemainingTime);
	KlingonHUD(PlayerPawn(Owner).myHUD).SetFragLimit(FragLimit);
*/
	if ( (Level.NetMode == NM_Standalone) || bMultiPlayerBots )
		RemainingBots = InitialBots;

	// Make sure all pawns are deleted from levels if they exist
	foreach AllActors(class 'Pawn',P) {
		if (P.IsA('KlingonPawn')) {
			P.Destroy();
		}
	}
...
function RestartGame()
{
	...
	Log(Self$".RestartGame() MapListType="$MapListType$" bChangeLevels="$bChangeLevels$" bAlreadyChanged="$bAlreadyChanged);
	...
	if ( bChangeLevels && !bAlreadyChanged && (MapListType != None) )
	{
		...
		Log(Self$".NextMap="$NextMap);

		if ( NextMap == "" )
			NextMap = GetMapName(MapPrefix, NextMap,1);
		if ( NextMap != "" )
		{
			Log(Self$".Level.ServerTravel("$NextMap$")");
...
defaultproperties
{
     ...
     ReSpawnEffect=Class'Klingons.TeleportParticles'
     TeleportSound=Sound'KlingonSFX01.Effects.Transporter'
     bNoMonsters=True
     bRestartLevel=False
     bPauseable=False
     ScoreBoardType=Class'Klingons.KlingonScoreBoard'
     GameMenuType=Class'Klingons.KlingonMenuDMGameOptions'
     MapListType=Class'Klingons.KlingonMapListDM'
     MapPrefix="DR"
}
